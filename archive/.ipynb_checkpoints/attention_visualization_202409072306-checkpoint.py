...
def concatenate_inputs_outputs(inputs: Dict[str, torch.Tensor], outputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    concatenated_dict = {}
...
def generate_output(prompt: str, model: PeftModel, tokenizer: AutoTokenizer) -> str:
...
def get_attention_map(prompt: str, model: PeftModel, tokenizer: AutoTokenizer) -> Tuple[List[List[torch.Tensor]], Dict[str, torch.Tensor], List[int]]:
    ...
    """
    completion: str = generate_output(prompt, model, tokenizer)
    
    inputs: Dict[str, torch.Tensor] = tokenizer(prompt, return_tensors="pt").to(model.device)
    outputs: Dict[str, torch.Tensor] = tokenizer(completion, return_tensors="pt").to(model.device)
    concatenates: Dict[str, torch.Tensor] = concatenate_inputs_outputs(inputs, outputs)
